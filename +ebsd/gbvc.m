classdef gbvc<matlab.mixin.Copyable%handle
    properties
...
%         end
        ...
        % function cp = copy(gv)
        %     ngv = numel(gv);
        %     cp = arrayfun( @(gv) ebsd.gbvc( gv.x, gv.y, gv.ID ), gv );
        %     for i = 1:ngv
        %         cp(i).vertice = gv(i).vertice;
        %         cp(i).isedge = gv(i).isedge;
        %     end
        % 
        %     % cp = ebsd.gbvc( gv.x, gv.y, gv.ID );
        %     % cp = SpecializedCopy;
        %     % cp.Prop1 = gv.Prop1;
